...
import { cn, toISOLocalDate } from '@/lib/utils'
...
export function ActivityCalendar({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const startDate = toISOLocalDate(new Date(year, month, 1))
        const endDate = toISOLocalDate(new Date(year, month + 1, 0))
  ...
  const goToToday = () => {
    ...
    // 如果当前选中的日期不是今天，则激活今天
    const selectedDateStr = selectedDate ? toISOLocalDate(selectedDate) : ''
    const todayStr = toISOLocalDate(today)
  ...
  const isDayActive = (dateStr: string): boolean => {
    ...
    const selectedDateStr = toISOLocalDate(selectedDate)
  ...
  const renderMonthView = () => {
    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      const dateStr = toISOLocalDate(date)
